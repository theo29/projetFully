...
//        Toast.makeText(this, "I receveid a community: " + community.toString(), Toast.LENGTH_LONG).show();
            ...
            TextView title_page = findViewById(R.id.new_community);
            title_page.setText(R.string.modify_community);

            EditText editText;
            editText = findViewById(R.id.nameC);
            editText.setText(gaeCommunity.getName());

            editText = findViewById(R.id.descriptionLongE);
    ...
    private void spinner_setCommunityType() {
        GAECommunityType gaeType;
        for (int i = 0; i < spin.getCount(); i++) {
            gaeType = (GAECommunityType) spin.getItemAtPosition(i);
            if (gaeType.getId().equals(gaeCommunity.getIdCommunityType())) {
                spin.setSelection(i);
                return;
            }
        }
    ...
    private void setSpinnerCommunityType() {
        spin = findViewById(R.id.spinner_community);
        ...
        ArrayAdapter<GAECommunityType> adapter = new ArrayAdapter<GAECommunityType>(this, R.layout.activity_community_edit, communityTypes) {
            ...
            public View getView(int position, View convertView, ViewGroup parent) {
                ...
                TextView textView1 = view.findViewById(R.id.textView_communitytypes);
...
//        gaeCommunity.setId(currentId);
        ...
        * Add new gaeCommunity, if the current mode is EDIT_MODE_NEW*/
        if (currentEditMode == EDIT_MODE_NEW) {
            ...
            GAECommunityType gaeType = (GAECommunityType) spin.getSelectedItem();
            gaeCommunity.setIdCommunityType(gaeType.getId());
